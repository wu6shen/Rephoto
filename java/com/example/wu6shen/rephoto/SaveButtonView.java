...
public class SaveButtonView extends Button {
    ...
    private PointF mPoint1, mPoint2, mPoint3;
    ...
    public SaveButtonView(Context context, AttributeSet attrs, int defStyle) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        canvas.drawLine(mPoint2.x, mPoint2.y, mPoint3.x, mPoint3.y, mSymbolPaint);
    }

    @Override
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        if (mViewWidth != 0) {
            mPoint1 = new PointF(mRadius / 2, mRadius / 2 * 2);
            mPoint2 = new PointF(mRadius / 2 * 2, mRadius / 2 * 3);
